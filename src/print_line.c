...
/*   Updated: 2022/01/13 18:02:07 by ajung            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	mfaible(t_all *all, int m)
{
	while (all->coor.droite.xA != all->coor.droite.xB)
	{
		my_mlx_pixel_put(all, all->coor.droite.xA, (all->coor.droite.xA * m + 0.5), 0x00FFFFFF);
...
void	mfort(t_all *all, int m)
{
	while (all->coor.droite.yA != all->coor.droite.yB)
	{
		my_mlx_pixel_put(all, (all->coor.droite.yA / m + 0.5), all->coor.droite.yA, 0x00FFFFFF);
		if (all->coor.droite.yA < all->coor.droite.yB)
			all->coor.droite.yA++;
		else if (all->coor.droite.yA > all->coor.droite.yB)
			all->coor.droite.yA--;
